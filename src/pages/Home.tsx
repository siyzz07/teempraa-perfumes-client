...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (hash === 'about') {
      setTimeout(() => document.getElementById('gallery')?.scrollIntoView({ behavior: 'smooth' }), 500);
    }
    if (hash === 'contact') {
...
END:VCARD`;
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#011a14] font-sans selection:bg-brand-accent selection:text-white overflow-x-hidden">
      <ProductDetailsModal 
      ...
      <HeroSection 
        ...
        onExplore={() => document.getElementById('horizontal-perfumes')?.scrollIntoView({ behavior: 'smooth' })}
        onBookAtelier={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
      />
