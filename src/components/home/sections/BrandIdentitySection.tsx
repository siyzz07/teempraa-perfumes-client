...
const BrandIdentity: React.FC<BrandIdentityProps> = ({ scrollYProgress, shopSettings, onSaveContact }) => {
  ...
  return (
    <section className="py-32 bg-[#022c22] px-6 relative overflow-hidden">
      ...
      >
         <img src="/mist_overlay.png" className="w-full h-full object-cover" alt="Mist" />
      </motion.div>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-24">
          ...
          >
            ...
            <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-brand-accent rounded-full flex flex-col items-center justify-center text-[#011a14] shadow-2xl rotate-12">
               <span className="text-[8px] font-black uppercase tracking-widest mb-1">Authentic</span>
               <span className="text-xl font-serif font-black italic">Emerald</span>
               <span className="text-[8px] font-black uppercase tracking-widest mt-1">Certified</span>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-4 mb-8">
              ...
              <span className="text-brand-accent font-black text-[10px] uppercase tracking-[0.4em]">Imperium</span>
            </div>
            <h2 className="text-5xl md:text-7xl font-serif font-black text-white mb-10 tracking-tighter leading-none">
              The Crown <br /> of <span className="text-brand-accent italic">Fragrance.</span>
            </h2>
            <p className="text-slate-300 text-lg md:text-xl leading-relaxed mb-12 font-light tracking-wide">
              {shopSettings?.description || 'Reserved for those of discerning taste. Our emerald collection uses rare essential oils derived from high-altitude botanicals, aged to perfection.'}
            </p>
            ...
            <div className="grid grid-cols-2 gap-10 mb-16">
              {[
                ...
                <div key={i} className="group cursor-default">
                  <span className="block text-3xl font-serif font-black text-white group-hover:text-brand-accent transition-colors">{stat.value}</span>
                  <span className="block text-[9px] uppercase tracking-[0.3em] font-black text-brand-accent/60">{stat.label}</span>
                </div>
              ))}
            </div>
